...
from keypoint_net import KeypointNet, KeypointNet_New
...
def train_model(model, output_uri, dataloader, loss_function, optimizer, scheduler, epochs, val_dataloader, intervals, input_size, num_kpt, save_checkpoints, kpt_keys, study_name, evaluate_mode):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            # Save model onnx for inference on TensorRT.
            if save_checkpoints:
                onnx_uri = os.path.join(output_uri,f"best_keypoints_{input_size[0]}x{input_size[1]}.onnx")
                ...
                print(f"Saving best model as ONNX file to {onnx_uri}... Done!")
                best_model = copy.deepcopy(model)

                torch_best_uri = os.path.join(output_uri,f"best_keypoints_{input_size[0]}x{input_size[1]}.pt")
                checkpoint_torch_best = {'epoch': epoch,
                              'model': model.state_dict(),
                              'optimizer': optimizer.state_dict()}
                torch.save(checkpoint_torch_best, torch_best_uri)
                print(f"Saving best model as PT file to {torch_best_uri}...Done!")
        else:
            tolerance += 1
        ...
        if save_checkpoints and epoch != 0 and (epoch + 1) % intervals == 0:
            ...
            print(f'Saving checkpoint model to {gs_pt_uri}... Done!')
        ...
        if tolerance >= max_tolerance:
            print(f"Training is stopped, loss no longer decreases. Epoch {best_epoch} has the best validation loss.")
            break
...
def print_kpt_L2_distance(model, dataloader, kpt_keys, study_name, evaluate_mode, input_size):
    kpt_distances = []
    if evaluate_mode:
        validation_textfile = open('D:\\Python Projects\\KPD\\RektNet\\logs\\rektnet_validation.txt', 'a+')
...
def main():
    ...
    # python train_eval.py --study_name l2_96 --input_size 96 --loss_type l2_softargmax --lr 0.05 --evaluate_mode

    parser.add_argument('--input_size', default=80, type=int, help='input image size')
    parser.add_argument('--train_dataset_uri', default='D:\\Python Projects\\KPD\\RektNet\\dataset\\rektnet_label.csv', help='training dataset csv directory path')
    ...
    parser.add_argument('--dataset_path', type=str, help='path to image dataset',default='D:\\Python Projects\\KPD\\RektNet\\dataset\\RektNet_Dataset')
    parser.add_argument('--loss_type', default='l1_softargmax', help='loss type: l2_softargmax|l2_heatmap|l1_softargmax')
    parser.add_argument('--validation_ratio', default=0.1, type=float, help='percent of dataset to use for validation')
    ...
    parser.add_argument('--lr_gamma', default=0.999, help='gamma for the scheduler') #0.999
    ...
    parser.add_argument('--geo_loss_gamma_vert', default=0.05, type=float, help='gamma for the geometric loss (horizontal)') #0.038
    parser.add_argument('--geo_loss_gamma_horz', default=0.05, type=float, help='gamma for the geometric loss (vertical)') #0.055
    ...
    add_bool_arg('evaluate_mode', default=True, help='whether to evaluate avg kpt mse vs BB size distribution at end of training')
    ...
    MODEL
    """
    model = KeypointNet_New(len(KPT_KEYS), INPUT_SIZE, onnx_mode=False)
