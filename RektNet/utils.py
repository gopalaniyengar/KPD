...
def prep_label(label, target_image_size, orig_image_size, image_path):
    ...
    for i in range(label.shape[0]):
        ...
        # print('orig image size', orig_image_size[0], orig_image_size[1])
        # print([int(row[1]), int(row[0])])
        try:
            x_tmp = int(row[1])
            y_tmp = int(row[0])
            hm_tmp[x_tmp, y_tmp] = 1.0
        except IndexError:
            if y_tmp == orig_image_size[1] and x_tmp == orig_image_size[0]:
                hm_tmp[x_tmp-1, y_tmp - 1] = 1.0
            elif y_tmp == orig_image_size[1]:
                hm_tmp[x_tmp, y_tmp-1] = 1.0
            elif x_tmp == orig_image_size[0]:
                hm_tmp[x_tmp-1, y_tmp] = 1.0
        hm[i] = cv2.resize(hm_tmp, target_image_size)
        hm[i] = cv2.GaussianBlur(hm[i], (5,5), 0)
        if hm[i].sum()==0:
            print("Incorrect Data Label Detected! Please revise the image label below and become the one with data!")
            print(image_path)
        hm[i] /= hm[i].sum()
    return hm
